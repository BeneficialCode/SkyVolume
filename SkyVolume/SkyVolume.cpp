...
HRESULT GetAudioSessionEnumerator(
    IMMDevice* pDevice,
    IAudioSessionEnumerator** ppSessionEnumerator
) {
    HRESULT hr = S_FALSE;
    IAudioSessionManager2* pSessionManager2 = nullptr;
    
    if (!pDevice || !ppSessionEnumerator) {
        return hr;
    }

    hr = pDevice->Activate(__uuidof(IAudioSessionManager2),
        CLSCTX_INPROC_SERVER, nullptr, (void**)&pSessionManager2);
    if (FAILED(hr)) {
        return hr;
    }

    hr = pSessionManager2->GetSessionEnumerator(ppSessionEnumerator);
    if (FAILED(hr)) {
        return hr;
    }
...
HRESULT GetSessionPid(IAudioSessionControl* pSessionControl, DWORD* pPid) {
    HRESULT hr = S_FALSE;
    IAudioSessionControl2* pSessionControl2 = nullptr;

    if (!pSessionControl || !pPid) {
        return hr;
    }
    ...
    hr = pSessionControl2->GetProcessId(pPid);
    if (FAILED(hr)) {
        return hr;
    }
...
HRESULT GetProcessVolume(
    ...
    ISimpleAudioVolume* pSessionSimpleVolume = nullptr;
    DWORD currentPid;
    ...
    for (int i = 0; i < sessionCount; i++) {
        ...
        hr = GetSessionPid(pSessionControl, &currentPid);
        if (FAILED(hr))
            continue;

        hr = pSessionControl->QueryInterface(
            __uuidof(ISimpleAudioVolume),
            (void**)&pSessionSimpleVolume
        );
        if (FAILED(hr)) {
            break;
        }
        ...
        float level = 0;
        hr = pSessionSimpleVolume->GetMasterVolume(&level);
        if (FAILED(hr))
            break;
...
DWORD GetProcPid(const wchar_t* exeName) {
    PROCESSENTRY32 pe32;
    HANDLE hProcessSnap;
    BOOL bMore;
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nShowCmd)
{
    ...
    DWORD pid = GetProcPid(L"Sky.exe");
    float level;
    ...
    hr = GetDefaultDevice(&pDevice);
    if (FAILED(hr))
        return 0;

    hr = GetAudioSessionEnumerator(pDevice, &pSessionEnumerator);
    if (FAILED(hr))
        return 0;
    ...
    ::RegisterClassExW(&wc);
    HWND hwnd = ::CreateWindowW(wc.lpszClassName, L"Sky:COTL Vulume Controller",
    ...
    while (!done)
    {
        ...
        {
            ImGuiWindowFlags flags = ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoMove | ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_AlwaysAutoResize;
            
            ImGui::Begin("Sky:Children Of The Light", nullptr, flags);
            ImGui::TextWrapped("Sky: Children Of The Light 音量控制");
            ImGui::Spacing();
